    ...
    >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {overview.sectorBreakdown.slice(0,6).map((s, i) => (
                  <div key={i} className="flex items-center justify-between p-3 bg-background rounded-[6px]">
                    <span className="text-[13px] text-text-primary">{s.sector || "Unset"}</span>
        ...
        <div className="space-y-4">
          ...
          <Card>
            {loading ? (
              ...
              <div className="py-12 text-center text-[13px] text-text-secondary">No SMEs found</div>
            ) : (
              <>
      ...
      {tab === "lenders" && (
        <Card title="Institutional Clients">
          {!lenders.length ? (
            <div className="py-12 text-center text-[13px] text-text-secondary">No lenders registered yet</div>
          ) : (
            <div className="divide-y divide-border">
              {lenders.map(l => (
                <div key={l.id} className="py-3 flex items-center gap-4">
                  ...
                  <div className="text-right">
                    <p className="text-[13px] font-bold text-primary">{l.portfolio_size || 0}</p>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[["green","Green (65–100)","success"],["amber","Amber (40–64)","warning"],["red","Red (0–39)","danger"]].map(([band, label, status]) => (
                      <div key={band} className="text-center p-4 bg-background rounded-[8px]">
                        ...
                        <p className="text-[24px] font-bold text-text-primary mt-2">{byBand[band]}</p>
                      </div>
                    ))}
                  </div>
                </Card>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={label} className="bg-surface border border-border rounded-[8px] p-3 text-center">
                    <p className="text-[11px] text-text-secondary">{label}</p>
                    <p className="text-[20px] font-bold text-primary mt-1">{value}</p>
                  </div>
                ))}
              </div>
            </Card>
