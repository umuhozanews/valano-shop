    ...
    >
              ...
              <div className="space-y-1.5">
                {overview.scoreTrend.slice(-7).map((r, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <span className="w-20 text-[11px] text-text-secondary">{r.day?.slice(5)}</span>
                    ...
                    <span className="w-10 text-[12px] text-text-primary font-medium">{Math.round(r.avg_score || 0)}</span>
                    <span className="w-8 text-[11px] text-text-secondary">{r.scored}×</span>
                  </div>
                ))}
              </div>
            </Card>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {overview.sectorBreakdown.slice(0,6).map((s, i) => (
                  <div key={i} className="flex items-center justify-between p-3 bg-background rounded-[6px]">
                    <span className="text-[11px] text-text-primary">{s.sector || "Unset"}</span>
                    <span className="text-[12px] font-bold text-primary">{s.count}</span>
                  </div>
                ))}
              </div>
            </Card>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            <div className="relative flex-1 max-w-xs">
              ...
              <input value={smeSearch} onChange={e => { setSmeSearch(e.target.value); setSmePage(1); }}
                ...
                className="w-full h-9 pl-9 pr-3 border border-border rounded-[6px] text-[11px] bg-surface focus:outline-none focus:ring-1 focus:ring-primary" />
            </div>
          </div>
          ...
          <Card>
            {loading ? (
              ...
              <div className="py-12 text-center text-[11px] text-text-secondary">No SMEs found</div>
            ) : (
              <>
                  ...
                  {smes.map(s => (
                    <div key={s.id} className="py-3 flex items-center gap-4">
                      <div className="flex-1 min-w-0">
                        <p className="text-[11px] font-medium text-text-primary">{s.name}</p>
                        <p className="text-[11px] text-text-secondary">{s.email} · {s.sector || "No sector"} · {s.district || "No district"}</p>
                      </div>
                      {s.score != null && (
                      ...
                      <button onClick={() => toggleActive(s.id, s.is_active)}
                        className={`px-2 py-1 text-[11px] rounded-[4px] font-medium ${s.is_active?"bg-success/10 text-success hover:bg-danger/10 hover:text-danger":"bg-danger/10 text-danger hover:bg-success/10 hover:text-success"}`}>
                ...
                {smeTotal > 20 && (
                  <div className="flex justify-center gap-2 mt-4">
                    ...
                    <span className="px-3 py-1.5 text-[12px] text-text-secondary">Page {smePage} of {Math.ceil(smeTotal/20)}</span>
      ...
      {tab === "lenders" && (
        <Card title="Institutional Clients">
          {!lenders.length ? (
            <div className="py-12 text-center text-[11px] text-text-secondary">No lenders registered yet</div>
          ) : (
            <div className="divide-y divide-border">
              {lenders.map(l => (
                <div key={l.id} className="py-3 flex items-center gap-4">
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-medium text-text-primary">{l.name}</p>
                    <p className="text-[11px] text-text-secondary">{l.email}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[11px] font-bold text-primary">{l.portfolio_size || 0}</p>
                    <p className="text-[11px] text-text-secondary">referred SMEs</p>
                  </div>
                  ...
                  <Card title="Scoring Activity (last 90 days)">
                    <div className="space-y-1.5">
                      {model.overTime.slice(-8).map((r, i) => (
                        <div key={i} className="flex items-center gap-3">
                          <span className="w-24 text-[11px] text-text-secondary">{String(r.week).slice(0,10)}</span>
                          <div className="flex-1 bg-border rounded-full h-2">
                            <div className="h-2 rounded-full bg-primary" style={{ width: `${Math.min(100, Number(r.avg)||0)}%` }} />
                          </div>
                          <span className="w-10 text-[12px] font-medium">{Math.round(Number(r.avg)||0)}</span>
                          <span className="w-8 text-[11px] text-text-secondary">{r.scored}×</span>
                        </div>
                      ))}
                    </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={label} className="bg-surface border border-border rounded-[8px] p-3 text-center">
                    <p className="text-[11px] text-text-secondary">{label}</p>
                    <p className="text-[18px] font-bold text-primary mt-1">{value}</p>
                  </div>
                ))}
              </div>
            </Card>
