  ...
  function CartPanel() {
    return (
      <div className="flex flex-col h-full">
        <h3 className="text-[12px] font-semibold text-text-primary mb-4">Current Sale</h3>
        {!cart.length ? (
          <div className="flex-1 flex items-center justify-center text-text-secondary text-[11px]">Cart is empty</div>
        ) : (
          <div className="flex-1 overflow-y-auto space-y-3 mb-4">
            {cart.map(item => (
              <div key={item.stock_item_id} className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-semibold text-text-primary w-20 text-right">{formatRWF(item.unit_price*item.quantity)}</span>
        ...
        <div className="border-t border-border pt-3 space-y-3">
          ...
          <input value={customer} onChange={e=>setCustomer(e.target.value)} placeholder="Customer name (optional)"
            className="w-full h-9 px-3 border border-border rounded-[6px] text-[11px] focus:outline-none focus:ring-1 focus:ring-primary" />
  ...
  return (
    <PageWrapper title="Point of Sale" subtitle="Record a new sale"
          ...
          <div className="mb-3">
            <div className="relative mb-3">
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by name or barcode…"
                className="w-full h-10 pl-9 pr-3 border border-border rounded-[6px] text-[11px] focus:outline-none focus:ring-1 focus:ring-primary bg-surface" />
            </div>
                  ...
                  return (
                    <div key={item.id}
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-[12px] font-bold text-primary">{formatRWF(item.sell_price_rwf)}</span>
                        <span className={`text-[11px] font-medium ${outOfStock?"text-danger":item.quantity<=item.low_stock_threshold?"text-warning":"text-text-secondary"}`}>
      ...
      <button onClick={() => setShowCart(true)}
        ...
        {cartCount > 0 && (
          <span className="absolute -top-1 -right-1 w-5 h-5 bg-danger text-white text-[11px] font-bold rounded-full flex items-center justify-center">{cartCount}</span>
        )}
      </button>
      ...
      <Modal open={showConfirm} onClose={() => setShowConfirm(false)} title="Confirm Sale"
        ...
        <div className="space-y-4">
          <p className="text-[11px] text-text-secondary">Customer: <strong>{customer || "Walk-in"}</strong></p>
          <p className="text-[11px] text-text-secondary">Payment: <strong>{PAYMENTS.find(p=>p.key===payment)?.label}</strong></p>
          <div className="flex justify-between py-2 border-t border-b border-border">
            ...
            <span className="text-[12px] font-black text-primary">{formatRWF(total)}</span>
          </div>

          <label className="flex items-center gap-2 text-[11px] font-medium text-text-primary cursor-pointer">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" min="0" value={amountPaid} onChange={e=>setAmountPaid(e.target.value)}
                    className="w-full h-9 px-2 border border-border rounded text-[11px] focus:outline-none focus:ring-1 focus:ring-primary" />
                </div>
                <div>
                  <label className="block text-[11px] font-semibold text-text-secondary uppercase mb-1">Remaining</label>
                  <div className="h-9 px-2 border border-border rounded text-[11px] flex items-center text-danger font-bold">
      ...
      {invoiceNum && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-surface rounded-[12px] p-8 max-w-sm w-full text-center shadow-xl">
            ...
            <p className="text-[11px] text-text-secondary mb-5">Invoice: {invoiceNum}</p>
