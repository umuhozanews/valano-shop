  ...
  return (
    <PageWrapper title={`Order #${id}`} subtitle={order.supplier_name}
            ...
            return (
              <div key={status} className="flex items-center flex-1 last:flex-none">
                <div className="flex flex-col items-center">
                  ...
                  <p className={`text-[11px] font-medium mt-1 ${current?"text-primary":done?"text-text-primary":"text-text-secondary"}`}>
      ...
      <Modal open={showStockIn} onClose={() => setShowStockIn(false)} title="Stock In Items"
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-[11px] font-medium text-text-primary mb-1">Assign to Branch</label>
          ...
          <div className="space-y-2">
            {stockItems.map((item, i) => (
              <div key={i} className="flex items-center justify-between gap-3">
                <span className="text-[11px] text-text-primary flex-1">{item.item_name} ({item.size} {item.color})</span>
