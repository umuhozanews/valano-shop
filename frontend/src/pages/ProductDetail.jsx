  ...
  if (!p) return (
    <div style={{ backgroundColor: "#F5F2ED", minHeight: "100vh" }}>
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-24 text-center">
        <p className="text-[16px] text-gray-500 mb-6">Product not found.</p>
        <Link to="/" className="text-[13px] font-bold tracking-wider" style={{ color: "#1B4332" }}>← Back to collection</Link>
      </div>
    </div>
  );
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-16">
          ...
          <div className="lg:pt-2">
            ...
            {/* Name */}
            <h1 className="text-[28px] sm:text-[36px] font-bold leading-tight text-[#1a1a1a] mb-4" style={{ fontFamily: "'Bebas Neue', sans-serif", letterSpacing: "0.02em" }}>
              {p.name}
            </h1>

            {/* Price */}
            <div className="flex items-baseline gap-3 mb-6">
              <span className="text-[28px] font-bold" style={{ color: "#1B4332" }}>{formatRWF(p.price)}</span>
              <span className="text-[16px] text-gray-400 line-through">{formatRWF(p.orig)}</span>
              {discount > 0 && (
            ...
            {/* Description */}
            <p className="text-[14px] text-gray-600 leading-relaxed mb-6">{p.desc}</p>
            ...
            <div className="flex gap-3 mb-4">
              ...
              <div className="flex items-center border rounded-sm overflow-hidden" style={{ borderColor: "#c8c2b8" }}>
                ...
                <span className="w-12 text-center text-[14px] font-semibold text-[#1a1a1a]">{qty}</span>
                <button
              ...
              <button
                ...
                className="flex-1 h-12 flex items-center justify-center gap-2 text-[13px] font-bold tracking-wider text-white rounded-sm transition-all"
              ...
              {related.map(r => (
                <Link key={r.id} to={`/product/${r.id}`} className="group cursor-pointer block">
                  <div className="relative overflow-hidden rounded-sm mb-3" style={{ aspectRatio: "4/5", backgroundColor: "#edeae5" }}>
                    ...
                    {r.orig > r.price && (
                      <span className="absolute top-2 left-2 text-[10px] font-bold bg-[#1B4332] text-white px-2 py-0.5 rounded-sm">SALE</span>
                    )}
                  </div>
                  <p className="text-[13px] font-medium text-[#1a1a1a] truncate mb-1">{r.name}</p>
                  <div className="flex items-center gap-2">
                    <span className="text-[12px] text-gray-400 line-through">{formatRWF(r.orig)}</span>
                    <span className="text-[14px] font-bold text-[#1a1a1a]">{formatRWF(r.price)}</span>
                  </div>
                </Link>
              ))}
            </div>
