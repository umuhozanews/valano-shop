    ...
    <div className="space-y-2 mb-4">
      {alerts.map((a, i) => (
        <div key={i} className="flex items-start gap-3 px-4 py-3 rounded-[6px] bg-warning/10 border border-warning/30">
          ...
          <p className="text-[11px] text-text-primary">{a.message}</p>
        </div>
      ))}
    </div>
  );
    ...
    >
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {lowStock.length ? lowStock.map(item => (
              <div key={item.id} className="flex items-center justify-between py-1">
                <p className="text-[11px] text-text-primary truncate flex-1">{item.name}</p>
                <span className={`text-[12px] font-bold ml-2 ${item.quantity === 0 ? "text-danger" : "text-warning"}`}>
                  {item.quantity === 0 ? "OUT" : item.quantity}
                </span>
              </div>
            )) : (
              <p className="text-[11px] text-success py-4 text-center">✓ All stock levels OK</p>
            )}
          </div>
          {lowStock.length > 0 && (
            <button onClick={() => navigate("/app/stock")}
      ...
      <Card title="Recent Activity">
        <div className="divide-y divide-border">
          ...
          {!activity.length && (
            <p className="text-[11px] text-text-secondary py-6 text-center">No recent activity</p>
          )}
        </div>
      </Card>
    </PageWrapper>
