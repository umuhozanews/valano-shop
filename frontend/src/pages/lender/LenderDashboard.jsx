  ...
  return (
    <div className="bg-white border border-border rounded-[10px] p-4">
      <div className="flex items-center justify-between mb-3">
        <p className="text-[11px] font-semibold text-text-primary">Portfolio Health</p>
      ...
      action={
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowRefForm(v => !v)}
            className="flex items-center gap-1.5 px-4 py-2 bg-primary text-white rounded-[6px] text-[11px] font-medium hover:bg-primary/90 transition-colors">
    ...
    >
      ...
      {showRefForm && (
        <div className="mb-5 bg-white border border-border rounded-[10px] shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-[12px] font-semibold text-text-primary">Refer an SME to Inzira Insights</p>
          ...
          <form onSubmit={handleReferral} className="flex flex-wrap gap-3 items-end">
            <div className="flex-1 min-w-[200px]">
              ...
              <input required type="email" value={refForm.sme_email}
                ...
                className="w-full h-9 px-3 border border-border rounded-[6px] text-[11px] focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" />
            </div>
            <div className="flex-1 min-w-[200px]">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setShowRefForm(false)}
                className="h-9 px-4 border border-border rounded-[6px] text-[11px] text-text-secondary hover:bg-background">
                Cancel
              </button>
              <button type="submit" disabled={submitting}
                className="h-9 px-4 bg-primary text-white rounded-[6px] text-[11px] font-medium hover:bg-primary/90 disabled:opacity-50">
      ...
      {tab === "portfolio" && (
        <div className="bg-white border border-border rounded-[10px] overflow-hidden">
          ...
          <div className="p-4 border-b border-border flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input value={search}
                ...
                className="w-full h-9 pl-9 pr-3 border border-border rounded-[6px] text-[11px] bg-background focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" />
            </div>
            <select value={bandFilter}
              onChange={e => { setBandFilter(e.target.value); setPage(1); }}
              className="h-9 px-3 border border-border rounded-[6px] text-[11px] bg-background focus:outline-none focus:ring-2 focus:ring-primary/30">
          ...
          {loading ? (
            <div className="py-16 text-center text-[11px] text-text-secondary">Loading portfolio…</div>
          ) : !clients.length ? (
            <div className="py-16 text-center">
              <Users size={40} className="mx-auto text-border mb-3" />
              <p className="text-[12px] font-semibold text-text-primary">No clients in your portfolio</p>
              <p className="text-[11px] text-text-secondary mt-1">Use the Refer SME button to add businesses</p>
            </div>
          ) : (
            <>
              {/* Table header */}
      ...
      {tab === "risk" && (
        <div className="space-y-3">
          {loading ? (
            <div className="py-16 text-center text-[11px] text-text-secondary">Loading…</div>
          ) : !dash?.atRisk?.length ? (
            <div className="bg-white border border-border rounded-[10px] py-16 text-center">
              <CheckCircle size={40} className="mx-auto text-green-500 mb-3" />
              <p className="text-[12px] font-semibold text-text-primary">No at-risk SMEs</p>
              <p className="text-[11px] text-text-secondary mt-1">All your clients score above 40</p>
            </div>
          ) : (
            <>
              ...
              {dash.atRisk.map((s, i) => (
                <div key={i} className="bg-white border border-red-200 rounded-[10px] p-4 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full bg-red-100 flex items-center justify-center shrink-0">
                    <span className="text-[11px] font-bold text-red-600">{s.score ?? "?"}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-semibold text-text-primary">{s.name}</p>
      ...
      {tab === "recent" && (
        <div className="bg-white border border-border rounded-[10px] overflow-hidden">
          <div className="px-4 py-3 border-b border-border bg-background">
            <p className="text-[11px] font-semibold text-text-primary">Latest Score Updates</p>
          ...
          {loading ? (
            ...
            <div className="py-12 text-center text-[11px] text-text-secondary">No scored SMEs yet</div>
          ) : (
            <div className="divide-y divide-border">
      ...
      {!loading && totalN === 0 && (
        <div className="mt-6 bg-orange-50 border border-orange-200 rounded-[10px] p-8 text-center">
          ...
          <p className="text-[12px] font-semibold text-text-primary">Start building your portfolio</p>
          ...
          <button onClick={() => setShowRefForm(true)}
            className="mt-4 inline-flex items-center gap-2 px-5 py-2.5 bg-primary text-white rounded-[8px] text-[11px] font-medium hover:bg-primary/90">
