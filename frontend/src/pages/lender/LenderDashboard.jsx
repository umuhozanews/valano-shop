...
function ScoreBadge({ score, band }) {
  if (score == null) return <span className="text-[11px] text-gray-400 italic">Unscored</span>;
  const dot = band === "green" ? "bg-green-500" : band === "amber" ? "bg-amber-500" : "bg-red-500";
  return (
    <span className={`inline-flex items-center gap-1.5 text-[12px] font-semibold px-2.5 py-0.5 rounded-full border ${BAND_BG[band] || "bg-gray-50 text-gray-600 border-gray-200"}`}>
  ...
  return (
    <div className="bg-white border border-border rounded-[10px] p-4 flex items-start gap-3">
      ...
      <div className="min-w-0">
        ...
        <p className="text-[24px] font-bold text-text-primary leading-tight mt-0.5">{value ?? "—"}</p>
        {sub && <p className="text-[11px] text-text-secondary mt-0.5">{sub}</p>}
      </div>
      {trend != null && (
        <div className={`ml-auto text-[11px] font-semibold ${trend >= 0 ? "text-green-600" : "text-red-500"}`}>
  ...
  return (
    <div className="bg-white border border-border rounded-[10px] p-4">
      <div className="flex items-center justify-between mb-3">
        <p className="text-[13px] font-semibold text-text-primary">Portfolio Health</p>
        <p className="text-[11px] text-text-secondary">{total} SMEs</p>
      </div>
      ...
      <div className="flex gap-4 mt-2">
        {[{ label:"Green", n: green, cls: "bg-green-500" }, { label:"Amber", n: amber, cls: "bg-amber-400" }, { label:"Red", n: red, cls: "bg-red-500" }].map(({ label, n, cls }) => (
          <div key={label} className="flex items-center gap-1.5">
            ...
            <span className="text-[11px] text-text-secondary">{label} <span className="font-semibold text-text-primary">{n}</span></span>
          </div>
        ))}
      </div>
    </div>
      ...
      action={
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowRefForm(v => !v)}
            className="flex items-center gap-1.5 px-4 py-2 bg-primary text-white rounded-[6px] text-[13px] font-medium hover:bg-primary/90 transition-colors">
    ...
    >
      ...
      {showRefForm && (
        <div className="mb-5 bg-white border border-border rounded-[10px] shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-[14px] font-semibold text-text-primary">Refer an SME to Inzira Insights</p>
              <p className="text-[12px] text-text-secondary mt-0.5">Enter the SME's registered email — they'll be added to your portfolio immediately.</p>
            </div>
          ...
          <form onSubmit={handleReferral} className="flex flex-wrap gap-3 items-end">
            <div className="flex-1 min-w-[200px]">
              <label className="text-[12px] font-medium text-text-secondary block mb-1">SME Email Address *</label>
              <input required type="email" value={refForm.sme_email}
                ...
                className="w-full h-9 px-3 border border-border rounded-[6px] text-[13px] focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" />
            </div>
            <div className="flex-1 min-w-[200px]">
              <label className="text-[12px] font-medium text-text-secondary block mb-1">Notes (optional)</label>
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setShowRefForm(false)}
                className="h-9 px-4 border border-border rounded-[6px] text-[13px] text-text-secondary hover:bg-background">
                Cancel
              </button>
              <button type="submit" disabled={submitting}
                className="h-9 px-4 bg-primary text-white rounded-[6px] text-[13px] font-medium hover:bg-primary/90 disabled:opacity-50">
      ...
      {tab === "portfolio" && (
        <div className="bg-white border border-border rounded-[10px] overflow-hidden">
          ...
          <div className="p-4 border-b border-border flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input value={search}
                ...
                className="w-full h-9 pl-9 pr-3 border border-border rounded-[6px] text-[13px] bg-background focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" />
            </div>
            <select value={bandFilter}
              onChange={e => { setBandFilter(e.target.value); setPage(1); }}
              className="h-9 px-3 border border-border rounded-[6px] text-[13px] bg-background focus:outline-none focus:ring-2 focus:ring-primary/30">
          ...
          {loading ? (
            <div className="py-16 text-center text-[13px] text-text-secondary">Loading portfolio…</div>
          ) : !clients.length ? (
            <div className="py-16 text-center">
              <Users size={40} className="mx-auto text-border mb-3" />
              <p className="text-[14px] font-semibold text-text-primary">No clients in your portfolio</p>
              <p className="text-[13px] text-text-secondary mt-1">Use the Refer SME button to add businesses</p>
            </div>
          ) : (
            <>
              {/* Table header */}
              <div className="hidden md:grid grid-cols-[1fr_120px_100px_140px_120px_80px] gap-3 px-4 py-2 bg-background border-b border-border">
                {["Business", "Sector", "Score", "Referral", "Referred On", ""].map(h => (
                  <p key={h} className="text-[11px] font-semibold uppercase tracking-wide text-text-secondary">{h}</p>
                ))}
              </div>
                ...
                {clients.map(c => {
                  ...
                  return (
                    <div key={c.id}
                      ...
                      {/* Sector */}
                      <p className="text-[12px] text-text-secondary hidden md:block truncate">{c.sector || "—"}</p>
                      {/* Score */}
                      ...
                      <div className="hidden md:flex items-center gap-1.5">
                        {c.referral_code ? (
                          <>
                            <span className={`inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full ${REF_COLOR[c.referral_status] || "bg-gray-100 text-gray-600"}`}>
                              <RefIcon size={10} />
                              {c.referral_status || "pending"}
                            </span>
                            <span className="text-[11px] text-text-secondary font-mono">{c.referral_code}</span>
                          </>
                        ) : (
                      ...
                      {/* Date */}
                      <p className="text-[11px] text-text-secondary hidden md:block">
                        {c.created_at ? new Date(c.created_at).toLocaleDateString() : "—"}
                      </p>
                      {/* Action */}
                      <div className="flex justify-end" onClick={e => { e.stopPropagation(); navigate(`/app/lender/sme/${c.sme_user_id}`); }}>
                        <span className="flex items-center gap-1 text-[12px] text-primary font-medium hover:underline">
              ...
              {total > 15 && (
                <div className="flex items-center justify-between px-4 py-3 border-t border-border bg-background">
                  <p className="text-[12px] text-text-secondary">Showing {(page-1)*15+1}–{Math.min(page*15, total)} of {total}</p>
                  <div className="flex gap-2">
                    <button disabled={page === 1} onClick={() => setPage(p => p - 1)}
                      className="px-3 py-1.5 border border-border rounded-[6px] text-[12px] disabled:opacity-40 hover:bg-background">← Prev</button>
                    <button disabled={page >= Math.ceil(total/15)} onClick={() => setPage(p => p + 1)}
                      className="px-3 py-1.5 border border-border rounded-[6px] text-[12px] disabled:opacity-40 hover:bg-background">Next →</button>
                  </div>
                </div>
              )}
            </>
      ...
      {tab === "risk" && (
        <div className="space-y-3">
          {loading ? (
            <div className="py-16 text-center text-[13px] text-text-secondary">Loading…</div>
          ) : !dash?.atRisk?.length ? (
            <div className="bg-white border border-border rounded-[10px] py-16 text-center">
              <CheckCircle size={40} className="mx-auto text-green-500 mb-3" />
              <p className="text-[14px] font-semibold text-text-primary">No at-risk SMEs</p>
              <p className="text-[13px] text-text-secondary mt-1">All your clients score above 40</p>
            </div>
          ) : (
            <>
              ...
              {dash.atRisk.map((s, i) => (
                <div key={i} className="bg-white border border-red-200 rounded-[10px] p-4 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full bg-red-100 flex items-center justify-center shrink-0">
                    <span className="text-[15px] font-bold text-red-600">{s.score ?? "?"}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] font-semibold text-text-primary">{s.name}</p>
                    <p className="text-[11px] text-text-secondary">{s.sector || "—"} · Score: {s.score ?? "unscored"}/100</p>
                  </div>
                  <div className="hidden sm:flex flex-col items-end gap-1">
                    <span className="text-[11px] px-2 py-0.5 rounded-full bg-red-100 text-red-700 font-medium">HIGH RISK</span>
                    <p className="text-[11px] text-text-secondary">Recommend advisory session</p>
                  </div>
                  <button
                    onClick={() => navigate(`/app/lender/sme/${s.sme_user_id || s.id}`)}
                    className="shrink-0 flex items-center gap-1 text-[12px] text-primary font-medium hover:underline">
      ...
      {tab === "recent" && (
        <div className="bg-white border border-border rounded-[10px] overflow-hidden">
          <div className="px-4 py-3 border-b border-border bg-background">
            <p className="text-[13px] font-semibold text-text-primary">Latest Score Updates</p>
            <p className="text-[12px] text-text-secondary">SMEs in your portfolio with the most recent scoring runs</p>
          </div>
          {loading ? (
            <div className="py-16 text-center text-[13px] text-text-secondary">Loading…</div>
          ) : !dash?.recentlyScored?.length ? (
            <div className="py-12 text-center text-[13px] text-text-secondary">No scored SMEs yet</div>
          ) : (
            <div className="divide-y divide-border">
              {dash.recentlyScored.map((s, i) => (
                <div key={i} className="px-4 py-3 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] font-semibold text-text-primary">{s.name}</p>
                    <p className="text-[11px] text-text-secondary">{s.sector || "—"}</p>
                  </div>
      ...
      {!loading && totalN === 0 && (
        <div className="mt-6 bg-orange-50 border border-orange-200 rounded-[10px] p-8 text-center">
          ...
          <p className="text-[16px] font-semibold text-text-primary">Start building your portfolio</p>
          ...
          <button onClick={() => setShowRefForm(true)}
            className="mt-4 inline-flex items-center gap-2 px-5 py-2.5 bg-primary text-white rounded-[8px] text-[13px] font-medium hover:bg-primary/90">
