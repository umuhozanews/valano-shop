...
export default function LenderSmeDetail() {
  ...
  if (loading) return (
    <div className="flex h-64 items-center justify-center text-text-secondary text-[11px]">Loading…</div>
  );
  if (!client) return (
    <div className="flex h-64 flex-col items-center justify-center gap-3">
      <AlertTriangle size={32} className="text-danger" />
      <p className="text-[12px] text-text-primary">SME not found in your portfolio</p>
      <button onClick={() => navigate("/app/lender")} className="text-[11px] text-primary hover:underline">← Back to portfolio</button>
    </div>
  );
    ...
    <PageWrapper
      ...
      action={
        <button onClick={() => navigate("/app/lender")}
          className="flex items-center gap-1.5 px-3 py-2 border border-border rounded-[6px] text-[11px] text-text-secondary hover:text-text-primary">
        ...
        <Card title="Credit Score" className="lg:col-span-1">
          {client.score != null ? (
            <div className="text-center py-4">
              <p className="text-[72px] font-bold leading-none text-text-primary">{client.score}</p>
              <div className="mt-2">
            ...
            <div className="text-center py-8">
              <p className="text-[12px] text-text-secondary">Not yet scored</p>
            </div>
          )}
        </Card>
        ...
        <Card title="Business Profile" className="lg:col-span-2">
          ...
          {client.notes && (
            <div className="mt-4 p-3 bg-background rounded-[6px]">
              ...
              <p className="text-[11px] text-text-primary">{client.notes}</p>
            </div>
          )}
              ...
              {scoreHistory.map((h, i) => {
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    ...
                    <span className="text-[11px] text-text-secondary">{h.created_at ? new Date(h.created_at).toLocaleDateString("en-RW", { month:"short", day:"numeric" }) : "—"}</span>
                  </div>
                );
              })}
            </div>
