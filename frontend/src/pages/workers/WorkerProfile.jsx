  ...
  return (
    <PageWrapper title={worker.name} subtitle={`${worker.role} · ${worker.branch_name}`}
      ...
      <Card className="mb-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h2 className="text-[18px] font-bold text-text-primary">{worker.name}</h2>
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-[11px] text-text-secondary">{worker.branch_name}</span>
            </div>
            <p className="text-[11px] text-text-secondary mt-1">{worker.email} · {worker.phone}</p>
          </div>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 mb-4">
        <Card><p className="text-[11px] text-text-secondary uppercase tracking-wide mb-1">Total Sales</p><p className="text-[26px] font-bold text-text-primary">{perf?.totalSales||0}</p></Card>
      ...
      <Card title="Monthly Target Progress" className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-[11px] text-text-secondary">{formatRWF(perf?.monthlyRevenue||0)} of {formatRWF(perf?.monthlyTarget||0)}</span>
          <span className="text-[11px] font-bold text-primary">{targetPct}%</span>
        </div>
            ...
            {perf.topItems.map((item,i) => (
              <div key={i} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div><p className="text-[11px] font-medium text-text-primary">{item.name}</p><p className="text-[11px] text-text-secondary">{item.units} units</p></div>
                </div>
                <span className="text-[11px] font-semibold text-primary">{formatRWF(item.revenue)}</span>
              </div>
            ))}
          </div>
        </Card>
