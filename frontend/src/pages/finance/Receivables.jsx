      ...
      {summary && (
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="bg-surface border border-border rounded-[8px] p-4">
            <p className="text-[13px] text-text-secondary">Total Outstanding</p>
            <p className="text-[22px] font-bold text-danger mt-1">{formatRWF(summary.total_outstanding)}</p>
          </div>
          <div className="bg-surface border border-border rounded-[8px] p-4">
            <p className="text-[13px] text-text-secondary">Overdue</p>
            <p className="text-[22px] font-bold text-warning mt-1">{summary.overdue_count ?? 0} records</p>
          </div>
          <div className="bg-surface border border-border rounded-[8px] p-4">
            <p className="text-[13px] text-text-secondary">Due in 7 Days</p>
            <p className="text-[22px] font-bold text-primary mt-1">{summary.due_soon ?? 0} records</p>
          </div>
        </div>
      )}
      ...
      {showForm && (
        <Card title="New Receivable" className="mb-4">
          <form onSubmit={handleCreate} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-[13px] text-text-secondary mb-1 block">Customer</label>
            ...
            <div>
              <label className="text-[13px] text-text-secondary mb-1 block">Amount (RWF)</label>
            ...
            <div>
              <label className="text-[13px] text-text-secondary mb-1 block">Due Date</label>
            ...
            <div className="sm:col-span-2 flex gap-2 justify-end">
              <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 border border-border rounded-[6px] text-[14px]">Cancel</button>
              <button type="submit" className="px-4 py-2 bg-primary text-white rounded-[6px] text-[14px] font-medium">Create</button>
            </div>
          </form>
        </Card>
      )}
      ...
      <div className="flex gap-1 border-b border-border mb-4">
        {["list", "aged"].map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 text-[14px] font-medium border-b-2 transition-colors capitalize ${tab === t ? "border-primary text-primary" : "border-transparent text-text-secondary hover:text-text-primary"}`}>
        ...
        <Card>
          ...
          {loading ? (
            ...
            <div className="py-12 text-center">
              ...
              <p className="text-[15px] font-medium text-text-primary">No receivables yet</p>
              <p className="text-[14px] text-text-secondary mt-1">Add a receivable to start tracking customer debts</p>
            </div>
          ) : (
                ...
                return (
                  <div key={r.id} className="py-3 flex items-center justify-between gap-4">
                    ...
                    <div className="text-right shrink-0 space-y-1">
                      ...
                      <p className="text-[13px] text-text-secondary">of {formatRWF(r.amount)}</p>
                    </div>
                        ...
                        paying?.id === r.id ? (
                          <div className="flex items-center gap-1">
                            <input type="number" value={payAmt} onChange={e => setPayAmt(e.target.value)}
                              className="w-24 h-7 border border-border rounded px-2 text-[13px]" placeholder="Amount" />
      ...
      {tab === "aged" && (
        <Card title="Aged Receivables Analysis">
          {loading ? (
            ...
            <div className="py-12 text-center text-[14px] text-text-secondary">No outstanding receivables</div>
          ) : (
