      ...
      {summary && (
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="bg-surface border border-border rounded-[8px] p-4">
            ...
            <p className="text-[18px] font-bold text-danger mt-1">{formatRWF(summary.total_outstanding)}</p>
          </div>
          <div className="bg-surface border border-border rounded-[8px] p-4">
            <p className="text-[11px] text-text-secondary">Overdue</p>
            <p className="text-[18px] font-bold text-warning mt-1">{summary.overdue_count ?? 0} records</p>
          </div>
          <div className="bg-surface border border-border rounded-[8px] p-4">
            <p className="text-[11px] text-text-secondary">Due in 7 Days</p>
            <p className="text-[18px] font-bold text-primary mt-1">{summary.due_soon ?? 0} records</p>
          </div>
        </div>
      )}
        ...
        <Card title="New Receivable" className="mb-4">
          <form onSubmit={handleCreate} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select required className="w-full border border-border rounded-[6px] px-3 py-2 text-[11px] bg-background"
            ...
            <div>
              ...
              <input required type="number" min="1" className="w-full border border-border rounded-[6px] px-3 py-2 text-[11px] bg-background"
            ...
            <div className="sm:col-span-2 flex gap-2 justify-end">
              <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 border border-border rounded-[6px] text-[11px]">Cancel</button>
              <button type="submit" className="px-4 py-2 bg-primary text-white rounded-[6px] text-[11px] font-medium">Create</button>
            </div>
          </form>
        </Card>
      )}
      ...
      <div className="flex gap-1 border-b border-border mb-4">
        {["list", "aged"].map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 text-[11px] font-medium border-b-2 transition-colors capitalize ${tab === t ? "border-primary text-primary" : "border-transparent text-text-secondary hover:text-text-primary"}`}>
        ...
        <Card>
          ...
          {loading ? (
            ...
            <div className="py-12 text-center">
              ...
              <p className="text-[12px] font-medium text-text-primary">No receivables yet</p>
              <p className="text-[11px] text-text-secondary mt-1">Add a receivable to start tracking customer debts</p>
            </div>
          ) : (
                  ...
                  <div key={r.id} className="py-3 flex items-center justify-between gap-4">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <p className="text-[11px] font-medium text-text-primary">{r.customer_name}</p>
                    ...
                    <div className="text-right shrink-0 space-y-1">
                      <p className="text-[11px] font-bold text-text-primary">{formatRWF(outstanding)}</p>
      ...
      {tab === "aged" && (
        <Card title="Aged Receivables Analysis">
          {loading ? (
            ...
            <div className="py-12 text-center text-[11px] text-text-secondary">No outstanding receivables</div>
          ) : (
