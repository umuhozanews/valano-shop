...
export default function ProfitLoss() {
  ...
  }, [viewMode, year, month, translate]);

  const activeData = viewMode === "monthly" ? monthlyData : dailyData;
  const t = activeData?.totals || {};
  
  // Calculate P&L Totals
  const grossProfit = (t.revenue || 0) - (t.cogs || 0);
  const netProfit = grossProfit - (t.expenses || 0);
  ...
  return (
    <PageWrapper title={translate("profit_loss")} subtitle={viewMode === "monthly" ? translate("monthly_summary") : "Daily P&L Performance"}
      ...
      <div className="flex border-b border-border mb-6">
        <button 
          ...
          className={`py-2.5 px-4 font-semibold text-[11px] border-b-2 transition-colors ${viewMode === "monthly" ? "border-primary text-primary" : "border-transparent text-text-secondary hover:text-text-primary"}`}
        >
          {translate("monthly_summary")}
        </button>
        <button 
          onClick={() => { setViewMode("daily"); }}
          className={`py-2.5 px-4 font-semibold text-[11px] border-b-2 transition-colors ${viewMode === "daily" ? "border-primary text-primary" : "border-transparent text-text-secondary hover:text-text-primary"}`}
        >
          Daily Breakdown
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-6 items-center">
        {viewMode === "daily" && (
          <select value={month} onChange={e=>setMonth(parseInt(e.target.value))} className="h-9 px-3 border border-border rounded-card text-[11px] bg-surface focus:outline-none focus:ring-2 focus:ring-primary font-medium">
            {MONTHS.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}
          </select>
        )}
        <select value={year} onChange={e=>setYear(parseInt(e.target.value))} className="h-9 px-3 border border-border rounded-card text-[11px] bg-surface focus:outline-none focus:ring-2 focus:ring-primary font-medium">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3 mb-6">
        ...
        <StatCard title={translate("gross_profit")} value={formatRWF(grossProfit)} />
        <StatCard title={translate("operating_expenses")} value={formatRWF(t.expenses||0)} />
        <StatCard title="Procurement Costs" value={formatRWF(t.procurement||0)} />
        <StatCard title={translate("net_profit")} value={formatRWF(netProfit)} />
      </div>
        ...
        <div className="space-y-6">
          {viewMode === "monthly" ? (
                  ...
                  <table className="w-full text-[11px]">
                    <tbody>
                      <tr className="bg-background">
                        <td className="py-2 px-3 font-semibold text-text-secondary uppercase text-[11px] tracking-wide">{translate("revenue")}</td>
                        <td />
                      </tr>
                      ...
                      <tr className="bg-background"><td className="py-2 px-3 font-semibold text-text-secondary uppercase text-[11px] tracking-wide">{translate("cogs")}</td><td /></tr>
                      <tr className="border-b border-border">
                      ...
                      <tr className="bg-background"><td className="py-2 px-3 font-semibold text-text-secondary uppercase text-[11px] tracking-wide">{translate("operating_expenses")}</td><td /></tr>
                      <tr className="border-b border-border">
                ...
                <Card title={translate("sales_trend")}>
                  <ResponsiveContainer width="100%" height={280}>
                    <AreaChart data={byMonth}>
                      ...
                      <Legend />
                      <Area type="monotone" dataKey="revenue" stroke="#10B981" fill="#D1FAE5" name={translate("revenue")} />
                      <Area type="monotone" dataKey="cogs" stroke="#F59E0B" fill="#FEF3C7" name={translate("cogs")} />
                      <Area type="monotone" dataKey="netProfit" stroke="#3B82F6" fill="#DBEAFE" name={translate("net_profit")} />
                    </AreaChart>
                  </ResponsiveContainer>
                </Card>
              </div>
                ...
                <Card title="Monthly Linked Business Activity" subtitle={`Summary of linked transactions for ${year}`}>
                  <div className="overflow-x-auto">
                    <table className="w-full text-[12px] text-left">
                      <thead>
                        <tr className="border-b border-border text-text-secondary uppercase text-[11px] tracking-wide bg-background">
                          <th className="py-2.5 px-3">Month</th>
                          <th className="py-2.5 px-3 text-right">Revenue</th>
                          <th className="py-2.5 px-3 text-right font-medium">COGS</th>
                          <th className="py-2.5 px-3 text-right">Expenses</th>
                          <th className="py-2.5 px-3 text-right">Procurements</th>
                          <th className="py-2.5 px-3 text-right">Net Profit</th>
                          <th className="py-2.5 px-3 text-center">Linked Activity Metrics</th>
                        </tr>
                      </thead>
                          ...
                          <tr key={idx} className="hover:bg-background/50 transition-colors">
                            ...
                            <td className="py-2.5 px-3 text-center">
                              <div className="flex justify-center gap-2">
                                <span className="px-2 py-0.5 bg-green-50 border border-green-200 text-green-700 rounded-btn text-[11px] font-medium">
                                  {m.sales_count} Sales • {m.customers_count} Customers
                                </span>
                                <span className="px-2 py-0.5 bg-amber-50 border border-amber-200 text-amber-700 rounded-btn text-[11px] font-medium">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              {/* Daily Trend Chart */}
              <div className="lg:col-span-12">
                <Card title={`Daily Profit & Loss Trend — ${getMonthName(month)} ${year}`}>
                  <ResponsiveContainer width="100%" height={260}>
                    <AreaChart data={byDay}>
                      ...
                      <Legend />
                      <Area type="monotone" dataKey="revenue" stroke="#10B981" fill="#D1FAE5" name={translate("revenue")} />
                      <Area type="monotone" dataKey="expenses" stroke="#EF4444" fill="#FEE2E2" name={translate("operating_expenses")} />
                      <Area type="monotone" dataKey="netProfit" stroke="#3B82F6" fill="#DBEAFE" name={translate("net_profit")} />
                    </AreaChart>
                  </ResponsiveContainer>
                </Card>
                    ...
                    <table className="w-full text-[12px] text-left">
                      <thead>
                        <tr className="border-b border-border text-text-secondary uppercase text-[11px] tracking-wide bg-background">
                          ...
                          <th className="py-2.5 px-3 text-right">Procurements</th>
                          <th className="py-2.5 px-3 text-right">Net Profit</th>
                          <th className="py-2.5 px-3 text-center">Linked Activity</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-border">
                        {dailyList.map((r, idx) => {
                                ...
                                <div className="flex justify-center gap-1.5 text-[11px]">
                                  ...
                                  {r.procurements_count > 0 && (
                                    <span className="px-1.5 py-0.5 bg-amber-50 border border-amber-200 rounded text-amber-700 font-medium">
                                      {r.procurements_count} Proc ({r.suppliers_count} Supp)
                                    </span>
                                  )}
                                  {r.sales_count === 0 && r.procurements_count === 0 && (
                                    <span className="text-text-secondary font-light">No activity</span>
                                  )}
                                </div>
                              </td>
                            </tr>
