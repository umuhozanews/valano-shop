    ...
    <PageWrapper title={t("debt_management")} subtitle={activeBusiness.name}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <Button variant="primary" size="lg" icon={Plus} onClick={() => setShowModal(true)} className="!h-auto flex-col gap-1 py-4">
          <span className="text-[11px]">{t("add_debt")}</span>
        </Button>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} rows={2}
              className="w-full p-2 border border-border rounded-card text-[11px] bg-surface focus:outline-none focus:ring-1 focus:ring-primary" />
          </div>
        </div>
      </Modal>
    </PageWrapper>
