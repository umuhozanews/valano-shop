      ...
      >
        <div className="max-w-[1400px] mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <Link to="/" className="shrink-0 flex flex-col leading-none">
            <span className="text-[13px] tracking-[0.2em] uppercase" style={{ color: "#1B4332", fontFamily: "Inter, sans-serif", fontStyle: "italic" }}>Fashion</span>
            ...
            <span className="text-[13px] font-bold tracking-[0.3em] uppercase text-center" style={{ color: "#1B4332" }}>SHOP</span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-8">
            {NAV_LINKS.map(item => (
              <Link key={item} to={`/?cat=${item.toLowerCase()}`}
                className="text-[14px] font-medium tracking-wider text-[#1a1a1a] hover:text-[#1B4332] transition-colors">
                {item}
              </Link>
            ))}
          </div>
          ...
          <div className="flex items-center gap-1">
            ...
            {searchOpen ? (
              <form onSubmit={handleSearch} className="flex items-center border rounded-sm overflow-hidden" style={{ borderColor: "#c8c2b8" }}>
                <input
                  ...
                  className="h-9 px-3 text-[14px] bg-transparent focus:outline-none w-44"
                  style={{ color: "#1a1a1a" }}
                />
            ...
            >
              ...
              {cartCount > 0 && (
                <span
                  className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full text-[11px] font-bold flex items-center justify-center text-white"
            ...
            <Link
              to="/app/login"
              className="hidden sm:inline-flex items-center gap-1.5 text-[13px] font-semibold tracking-wider border-2 px-4 py-1.5 rounded-sm transition-colors ml-1"
          ...
          <div className="md:hidden border-t px-6 py-4 space-y-3" style={{ borderColor: "#e5e0d8" }}>
            {NAV_LINKS.map(item => (
              <Link key={item} to={`/?cat=${item.toLowerCase()}`} onClick={() => setMenuOpen(false)}
                className="block text-[15px] font-medium py-1" style={{ color: "#1a1a1a" }}>
                {item}
              </Link>
            ))}
            <Link to="/app/login" className="block w-full text-center mt-3 py-2 border-2 text-[14px] font-bold tracking-wider rounded-sm"
              style={{ borderColor: "#1B4332", color: "#1B4332" }}>
              STAFF LOGIN
            </Link>
          </div>
