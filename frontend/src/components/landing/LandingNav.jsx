          ...
          <div className="hidden md:flex items-center gap-8">
            {NAV_LINKS.map(item => (
              <Link key={item} to={`/?cat=${item.toLowerCase()}`}
                className="text-[11px] font-medium tracking-wider text-[#1a1a1a] hover:text-[#1B4332] transition-colors">
                {item}
              </Link>
            ))}
          </div>
            ...
            {searchOpen ? (
              <form onSubmit={handleSearch} className="flex items-center border rounded-sm overflow-hidden" style={{ borderColor: "#c8c2b8" }}>
                <input
                  ...
                  className="h-9 px-3 text-[11px] bg-transparent focus:outline-none w-44"
                  style={{ color: "#1a1a1a" }}
                />
          ...
          <div className="md:hidden border-t px-6 py-4 space-y-3" style={{ borderColor: "#e5e0d8" }}>
            {NAV_LINKS.map(item => (
              <Link key={item} to={`/?cat=${item.toLowerCase()}`} onClick={() => setMenuOpen(false)}
                className="block text-[12px] font-medium py-1" style={{ color: "#1a1a1a" }}>
                {item}
              </Link>
            ))}
            <Link to="/app/login" className="block w-full text-center mt-3 py-2 border-2 text-[11px] font-bold tracking-wider rounded-sm"
              style={{ borderColor: "#1B4332", color: "#1B4332" }}>
              STAFF LOGIN
            </Link>
          </div>
