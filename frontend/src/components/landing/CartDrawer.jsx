      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b" style={{ borderColor: "#e5e0d8" }}>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[12px] font-bold tracking-wider" style={{ color: "#1B4332" }}>
              CART {cart.length > 0 && <span className="text-[11px] font-normal text-gray-500">({cart.reduce((s,i)=>s+i.quantity,0)} items)</span>}
            </h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center gap-4 py-16">
              ...
              <p className="text-[12px] text-gray-400">Your cart is empty</p>
              <button
                onClick={onClose}
              ...
              {cart.map(item => (
                <div key={item.key} className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[12px] font-bold mt-1" style={{ color: "#1B4332" }}>{formatRWF(item.price)}</p>
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-[11px] font-bold text-[#1a1a1a]">{formatRWF(item.price * item.quantity)}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {cart.length > 0 && (
          <div className="px-6 py-5 border-t space-y-4" style={{ borderColor: "#e5e0d8" }}>
            <div className="flex items-center justify-between">
              <span className="text-[11px] text-gray-500 tracking-wider uppercase">Subtotal</span>
            ...
            <button
              ...
              className="w-full h-12 flex items-center justify-center gap-2.5 text-[11px] font-bold tracking-wider text-white rounded-sm transition-colors"
